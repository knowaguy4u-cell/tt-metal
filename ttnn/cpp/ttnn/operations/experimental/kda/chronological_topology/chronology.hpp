...
struct Topology {
    ...
    uint32_t reset_chunk(uint32_t group, uint32_t groups) const {
        return has_valid_tail() && group == wrap_group(groups) ? split_in_group(groups) : 0;
    }
    uint32_t reset_group(uint32_t groups) const {
        return has_valid_tail() ? wrap_group(groups) + uint32_t(split_in_group(groups) != 0) - 1 : groups;
    }
...
inline Topology derive_interval(uint32_t start, uint32_t end, uint32_t rank, uint32_t partitions, uint32_t rows) {
    ...
    const uint32_t step = (rank + partitions - t.boundary) % partitions;
    ...
    t.final_owner = (t.boundary + last_step) % partitions;
