...
def test_padding_preparation_and_scan(device, device_params):
    ...
    def run(start_bound=start, end_bound=end):
        ...
        output, state = ttnn.experimental.kda.recurrent_chunk_scan(
            *prepared, initial_tt, actual_start=start_bound, actual_end=end_bound
        )
    ...
    try:
        for length in (256, 32, 96, 160, 32, 256):
            ...
            expected_terms = ttnn.experimental.kda.prepare_chunk_recurrence(*trimmed, trimmed_beta, heads)
            expected_output, expected_state = ttnn.experimental.kda.recurrent_chunk_scan(*expected_terms, initial_tt)
            ...
            for observed, expected in zip(outputs[9:], expected_summary, strict=True):
            ...
            for observed, expected in zip(rebound[9:], expected_summary, strict=True):
                torch.testing.assert_close(ttnn.to_torch(observed), ttnn.to_torch(expected).bfloat16(), rtol=0, atol=0)
