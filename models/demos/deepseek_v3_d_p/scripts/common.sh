...
# GLM only: the tp_shard_kv axis (KV dedup across the sp*tp cache striping). The Kimi test has no
# such axis, so this token appears in the GLM node id only.
TP_ID="${TP_ID:-sp_only}"
...
case "${MODEL:-}" in
  ...
  GLM5_2)
    ...
    # The GLM test has no perf_margin param, so its node id has no margin token; it does have a
    # tp_shard_kv axis, whose token sits right after the variant.
    VARIANT_ID="glm52-$TP_ID"; LAYERS_ID="L78"; NODE_SUFFIX="-$TRACE_ID"
